...
export default {
  ...
  getElixirExecutableSetting() {
    return atom.config.get("atom-elixir-formatter-pulsar.elixirExecutable");
  },
  ...
  isPackageEnabled() {
    return !atom.packages.isPackageDisabled("atom-elixir-formatter-pulsar");
  },

  // returns true if files should be formatted on save
  shouldFormatOnSave() {
    switch (atom.config.get("atom-elixir-formatter-pulsar.formatOnSave")) {
      case "always":
        return true;
        break;
  ...
  upgradeSettings() {
    ...
    atom.config.unset("atom-elixir-formatter-pulsar.mixExecutable");
  }
};
